...
pub fn create<P>(path: P, erase: bool) -> crate::bundler::Result<()>
where
    P: AsRef<Path>,
{
...
/// erasing it first if specified.
pub fn create_all<P>(path: P, erase: bool) -> crate::bundler::Result<()>
where
    P: AsRef<Path>,
{
    if erase && path.as_ref().exists() {
...
pub fn remove<P: AsRef<Path>>(path: P) -> crate::bundler::Result<()> {
...
pub fn copy_file<P, Q>(from: P, to: Q, options: &FileOpts) -> crate::bundler::Result<u64>
where
    P: AsRef<Path>,
    Q: AsRef<Path>,
{
    ...
    if !from.exists() {
        if let Some(msg) = from.to_str() {
            ...
            return Err(crate::bundler::Error::PathUtil(msg));
        }
        return Err(crate::bundler::Error::PathUtil(
...
#[allow(dead_code)]
pub fn copy<P, Q>(from: P, to: Q, options: &Options) -> crate::bundler::Result<u64>
where
    P: AsRef<Path>,
    Q: AsRef<Path>,
{
    ...
    for file in dir_content.files {
        ...
        let mut result_copy: crate::bundler::Result<u64>;
        let mut work = true;
        ...
        while work {
            ...
            match result_copy {
                ...
                Err(err) => {
                    ...
                    return Err(crate::bundler::Error::PathUtil(err_msg));
                }
            }
        }
    }
...
pub fn get_dir_info<P>(path: P, options: &DirOpts) -> crate::bundler::Result<DirInfo>
where
    P: AsRef<Path>,
{
...
fn _get_dir_info<P>(path: P, mut depth: u64) -> crate::bundler::Result<DirInfo>
where
    P: AsRef<Path>,
{
    ...
    if item.is_none() {
        return Err(crate::bundler::Error::PathUtil("Invalid Path".to_owned()));
    }
