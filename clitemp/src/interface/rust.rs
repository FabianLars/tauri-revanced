...
use crate::bundler::bundle::{
    AppCategory, AppImageSettings, BundleBinary, BundleSettings, DmgSettings, MacOsSettings,
...
use self::manifest::{read_manifest, rewrite_manifest};

use super::{AppSettings, DevProcess, ExitReason, Interface};
use crate::{
    helpers::{
        app_paths::{app_dir, tauri_dir},
        config::{reload as reload_config, BundleResources, Config},
    },
    ConfigValue,
};
...
impl Interface for Rust {
    ...
    fn new(config: &Config, target: Option<String>) -> crate::Result<Self> {
        let manifest = {
            ...
            let (manifest, _) = rewrite_manifest(config)?;
            let now = Instant::now();
        ...
        match self {
          ...
          "error inheriting `{label}` from workspace root manifest's `workspace.package.{label}`"
            ...
            MaybeWorkspace::Workspace(TomlWorkspaceField { workspace: false }) => Err(
                anyhow::anyhow!("`workspace=false` is unsupported for `package.{}`", label,),
            ),
        }
    }
            ...
            for entry in fs_bins {
                ...
                if let Some(name) = path.file_stem() {
                    let bin_exists = binaries.iter().any(|bin| {
                        bin.name() == name
                            || path.ends_with(bin.src_path().unwrap_or(&"".to_string()))
                    });
                    if !bin_exists {
                    ...
                    cargo_package_settings
                        ...
                        .resolve("version", || {
                            ws_package_settings
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!(
                                        "Couldn't inherit value for `version` from workspace"
                                    )
                                })
                        })
                    ...
                    homepage
                        .resolve("homepage", || {
                            ws_package_settings
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!(
                                        "Couldn't inherit value for `homepage` from workspace"
                                    )
                                })
                        })
                        .unwrap()
                    ...
                    authors
                        .resolve("authors", || {
                            ws_package_settings
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!(
                                        "Couldn't inherit value for `authors` from workspace"
                                    )
                                })
                        })
                        .unwrap()
                    ...
                    license
                        .resolve("license", || {
                            settings
                                ...
                                .ok_or_else(|| {
                                    anyhow::anyhow!(
                                        "Couldn't inherit value for `license` from workspace"
                                    )
                                })
                        })
                        .unwrap()
