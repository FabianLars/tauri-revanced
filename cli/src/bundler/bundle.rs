...
pub use self::{
    ...
    settings::{
        AppImageSettings, BundleBinary, BundleSettings, DmgSettings, MacOsSettings,
        PackageSettings, PackageType, Position, Settings, SettingsBuilder, Size, UpdaterSettings,
    },
};
#[cfg(target_os = "macos")]
use anyhow::Context;
...
pub fn bundle_project(settings: Settings) -> super::Result<Vec<Bundle>> {
        ...
        let bundle_paths = match package_type {
            ...
            PackageType::Updater => {
                if !package_types.iter().any(|p| {
                    matches!(
                        p,
                        PackageType::AppImage | PackageType::MacOsBundle | PackageType::Dmg
                    )
                }) {
    ...
    if !bundles.is_empty() {
        ...
        for bundle in &bundles {
            for path in &bundle.bundle_paths {
                let mut note = "";
                if bundle.package_type == PackageType::Updater {
                    note = " (updater)";
                }
...
pub fn check_icons(settings: &Settings) -> super::Result<bool> {
