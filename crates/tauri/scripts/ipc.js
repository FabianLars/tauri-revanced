...
(function () {
    ...
    function isIsolationMessage(event) {
        if (typeof event.data === 'object' && typeof event.data.payload === 'object') {
            ...
            return keys.length > 0 && keys.every((key) => key === 'nonce' || key === 'payload');
        }
        return false;
    }
    ...
    function sendIsolationMessage(data) {
        ...
        if (!isolation.frame) {
            ...
            if (frame.src.startsWith(isolationOrigin)) {
                ...
                console.error('Tauri IPC found an isolation iframe, but it had the wrong origin');
            }
        }

        // ensure we have the target to send the message to
        if (!isolation.frame || !isolation.frame.contentWindow) {
            console.error('Tauri "Isolation" Pattern could not find the Isolation iframe window');
            return;
        }
    ...
    Object.defineProperty(window.__TAURI_INTERNALS__, 'ipc', {
        ...
        value: Object.freeze((message) => {
            switch (pattern) {
                ...
                default:
                    console.error('Tauri IPC did not find a Tauri Pattern that it understood.');
                    break;
            }
        }),
    });
