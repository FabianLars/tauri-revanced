            ...
            Method::OPTIONS => {
                ...
                r.headers_mut().insert(
                    ...
                    HeaderValue::from_static(
                        "Content-Type, Tauri-Callback, Tauri-Error, Tauri-Channel-Id",
                    ),
                );
                respond(r);
            }
                        ...
                        .and_then(|message| {
                            Ok(Message {
                                ...
                                payload: serde_json::from_slice(
                                    &crypto_keys.decrypt(message.payload)?,
                                )?,
                            ...
                            && cmd != crate::ipc::channel::FETCH_CHANNEL_DATA_COMMAND
                        {
                            fn responder_eval<R: Runtime>(
                                ...
                                let eval_js = match js {
                                    Ok(js) => js,
                                    Err(e) => format_callback(error, &e.to_string()).expect(
                                        "unable to serialize response error string to json",
                                    ),
                                };
                            ...
                            let _response_span = tracing::trace_span!(
                                ...
                                mime_type = match &response {
                                    InvokeResponse::Ok(InvokeBody::Json(_)) =>
                                        mime::APPLICATION_JSON,
                                    InvokeResponse::Ok(InvokeBody::Raw(_)) =>
                                        mime::APPLICATION_OCTET_STREAM,
                            ...
                            match &response {
                                InvokeResponse::Ok(InvokeBody::Json(v)) => {
                                    ...
                                    {
                                        let _ =
                                            Channel::from_callback_fn(webview, callback).send(v);
                                    } else {
                                        responder_eval(
                                            &webview,
