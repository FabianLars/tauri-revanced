        ...
        input
            ...
            .filter_map(|(prefix, entry)| {
                match entry {
                    ...
                    Ok(entry) => {
                        if let Err(error) = csp_hashes.add_if_applicable(
                            &entry,
                            &options.dangerous_disable_asset_csp_modification,
                        ) {
                            Some(Err(error))
                        } else {
...
impl EmbeddedAssets {
    ...
    pub fn new(
        ...
        let RawEmbeddedAssets { paths, csp_hashes } =
            RawEmbeddedAssets::new(input.into(), options)?;
